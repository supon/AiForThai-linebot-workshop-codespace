...
# This original code file is part of the AIForThai project, which provides various NLP and speech processing functionalities.
# The code is designed to work with the LINE Messaging API to process text and audio messages using
# AIForThai's NLP capabilities, including tokenization, translation, sentiment analysis, and more
# For Partii STT
import re
...
def handle_text_message(event):
    ...
    if matched_command:
        ...
        if matched_command == "#trexplus":
            ...
            # result = send_message(event, str(callG2P(content)))
            ...
            text = callTextSummarization(content)
            # send_message(event, text)
            if text:
                response = callVaja9(text, 0)
                speaker = 0
                ...
                send_message(event, "Text summarization failed")
...
def callTextSummarization(content):
    ...
    response = requests.post(url, data=params, headers=headers)
    txt = response.text
    src_tag = bytes(txt, "utf-8").decode("unicode_escape")
    # print(src_tag)
    if response.status_code == 200:
        return src_tag
    else:
